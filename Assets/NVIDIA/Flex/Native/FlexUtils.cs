...
namespace NVIDIA.Flex
{
    public static class FlexUtils
    {
        ...
        struct ToBytes<T> where T : struct
        {
            [FieldOffset(0)] public byte[] byteArray;
            [FieldOffset(0)] public T[] otherArray;
        }
        public static void SafeFastCopy<T>(T[] source, IntPtr destination) where T : struct
        {
            ToBytes<T> toBytes = new ToBytes<T>() { otherArray = source };
        ...
        {
            ToBytes<T> toBytes = new ToBytes<T>() { otherArray = destination };
